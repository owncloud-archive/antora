...
def prepublish(config):
  return [{
    ...
    'settings': {
      ...
      'repo': 'registry.drone.owncloud.com/owncloudci/antora',
...
def sleep(config):
  return [{
    ...
    'commands': [
      'retry -- reg digest --username $DOCKER_USER --password $DOCKER_PASSWORD registry.drone.owncloud.com/owncloudci/antora:%s' % config['internal'],
    ],
  }]
...
def trivy(config):
  ...
  return [
    ...
    },
    {
      ...
      'commands': [
        'retry -- trivy registry.drone.owncloud.com/owncloudci/antora:%s' % config['internal'],
      ],
    },
  ]
...
def cleanup(config):
  return [{
    ...
    'commands': [
      'reg rm --username $DOCKER_USER --password $DOCKER_PASSWORD registry.drone.owncloud.com/owncloudci/antora:%s' % config['internal'],
    ],
    'when': {
      'status': [
        'success',
